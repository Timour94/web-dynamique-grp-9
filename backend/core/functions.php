...
function create_product(array $data, int $sellerId, ?string $imagePath = null): array
{
    $pdo = getDB();
    $errors = [];
    ...
    try {
        ...
        if ($imagePath) {
            $img = $pdo->prepare('INSERT INTO product_images (id_product, chemin_image, image_principale) VALUES (?, ?, 1)');
...
function update_product(int $productId, array $data, int $sellerId, ?string $imagePath = null): array
{
    ...
    $pdo = getDB();
    $stmt = $pdo->prepare('UPDATE products SET id_categorie = ?, titre = ?, description = ?, prix_achat_immediat = ?, etat_produit = ?, stock = ?, statut = IF(statut = "refuse", "en_attente", statut), date_modification = NOW() WHERE id_product = ?');
    $stmt->execute([$idCategorie, $titre, $description, $prixAchat, $etat, $stock, $productId]);
    if ($imagePath) {
        $pdo->prepare('UPDATE product_images SET image_principale = 0 WHERE id_product = ?')->execute([$productId]);
        $pdo->prepare('INSERT INTO product_images (id_product, chemin_image, image_principale) VALUES (?, ?, 1)')->execute([$productId, $imagePath]);
    }
...
function place_bid(int $auctionId, int $userId, float $amount): array
{
    ...
    $pdo->beginTransaction();
    try {
        $stmt = $pdo->prepare('SELECT a.*, p.id_vendeur, p.titre, p.statut FROM auctions a JOIN products p ON p.id_product = a.id_product WHERE a.id_auction = ? FOR UPDATE');
        ...
        $pdo->prepare('INSERT INTO bids (id_auction, id_user, montant, date_bid) VALUES (?, ?, ?, NOW())')->execute([$auctionId, $userId, $amount]);
        $pdo->prepare('UPDATE auctions SET prix_actuel = ? WHERE id_auction = ?')->execute([$amount, $auctionId]);
        add_notification((int)$auction['id_vendeur'], 'Nouvelle enchère', 'Une offre de ' . money_format_fr($amount) . ' a été placée sur "' . $auction['titre'] . '".', 'enchere', 'enchere.php?id=' . (int)$auction['id_product']);
        $pdo->commit();
        return ['success' => true, 'message' => 'Offre enregistrée.'];
    } catch (Throwable $e) {
        $pdo->rollBack();
        return ['success' => false, 'message' => $e->getMessage()];
    }
...
function send_counter_offer(int $negotiationId, int $senderId, float $offer, string $message): array
{
    ...
    add_notification($receiver, 'Contre-offre reçue', 'Une nouvelle offre a été envoyée pour "' . $neg['titre'] . '".', 'negociation', 'negotiation.php?id=' . $negotiationId);
    return ['success' => true, 'message' => 'Contre-offre envoyée.'];
}
